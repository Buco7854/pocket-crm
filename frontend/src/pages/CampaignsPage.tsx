...
export default function CampaignsPage() {
  ...
  const [emailCreateTrigger, setEmailCreateTrigger] = useState(0)
  ...
  const loadCampaigns = useCallback(() => {
    ...
    if (filterValues.status) filters.push(`status = "${filterValues.status}"`)
    if (filterValues.type) filters.push(`type = "${filterValues.type}"`)
    campaignsCollection.fetchList({ page, sort: '-created', filter: filters.join(' && ') })
  }, [campaignsCollection.fetchList, page, debouncedSearch, filterValues.status, filterValues.type])
  ...
  const otherFilters: FilterOption[] = [
    {
      key: 'type',
      labelKey: 'campaign.type',
      options: (['ads', 'social', 'event', 'seo', 'autre'] as CampaignType[]).map((v) => ({
        value: v,
        label: t(`campaignType.${v}`),
      })),
    },
    {
      key: 'status',
      labelKey: 'fields.status',
      options: (['brouillon', 'en_cours', 'termine'] as CampaignStatus[]).map((s) => ({
        value: s,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {activeTab === 'email' ? (
          <Button icon={<Plus className="h-4 w-4" />} onClick={() => setEmailCreateTrigger((n) => n + 1)}>
            {t('campaign.addEmail')}
          </Button>
        ) : (
      ...
      {/* ── Email campaigns tab ── */}
      {activeTab === 'email' && <EmailCampaignList createTrigger={emailCreateTrigger} />}
